...
$extra = ' AND derivacion.SIT_ID = '.Configuration::enAreaGeneradora;
$config["catalogos"]["area_analisis"] = helperCatalogos::getAreasAnalisis(2);
//echo "hola";exit;
$data = helperMuestra::getAllDerivacionesOrigen($config, $extra, $config["catalogos"]["area_analisis"]);
//print_r($data);exit;
$config["total"] = helperMuestra::getCountAllDerivacionesOrigen($config, $extra, $config["catalogos"]["area_analisis"]);
...
$result.= '
            <tr id="0">
                ...
                <th class="dxgvHeader_PlasticBlue" scope="">Fecha Derivaci&oacute;n</th>
           ...
           if (is_array($data))
           {
                foreach ($data as $data)
                {
                    ...
                    $result.='<td class="dxgv" name="global" width="auto" id="g'.$idMuestra.'">
                                    '.$data["global"].' - '.helperString::completeZeros($data["gnumero"]).'
                                </td>
                                <td class="dxgv" name="correlativo" width="auto" id="c'.$idMuestra.'">
                                    '.$data["correlativo"].' - '.helperString::completeZeros($data["cnumero"]).'
                                </td>
                                <td class="dxgv" width="20px">
                ...
                $result.='<tr id="noResults" class="dxgvDataRow_PlasticBlue"><td class="dxgv" align="center" colspan="6">B&uacute;squeda sin resultados.</td></tr>';
