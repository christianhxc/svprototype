...
require_once("libs/helper/helperLdbiNotas.php");
...
$data = helperLdbiNotas::buscar($config);
$config["total"] = helperLdbiNotas::buscarCantidad($config);
...
$result.= '
            <tr>
                ...
                <th class="dxgvHeader_PlasticBlue" scope="">Fecha nota</th>
                <th class="dxgvHeader_PlasticBlue" scope="">Nota</th>
...
if (is_array($data)) {
    foreach ($data as $data) {
        ...
        if ($permisoBorrar) {
            $borrarUceti = '<a href="javascript:borrar(' . $data["id_nota"] . ',\'' . $data["no_nota"] . '\')" class="" title="Borrar">'
                . '<img title="Borrar" border=0 src="../img/Delete.png"></a>';
        }
        $result.='<tr class="dxgvDataRow_PlasticBlue">';
        $result.='
                                <td class="dxgv" width="2%">
                                    ' . ($data["id_nota"] == "" ? "Sin datos" : htmlentities($data["id_nota"])) . '
                                </td>
                                <td class="dxgv" width="3%">
                                    ' . ($data["fh_nota"] == "" ? "No disponible" : helperString::toDateView($data["fh_nota"])) . '
                                </td>
                                <td class="dxgv" width="5%">
                                    ' . ($data["no_nota"] == "" ? "No disponible" : htmlentities($data["no_nota"])) . '
                                </td>
                                ...
                                <td style="text-align: center" class="dxgv" width="3%">
                                    ' . ($data["fh_nota"] == "" ? "No disponible" : substr($data["fh_nota"],0,4)) . '
                                </td>
                                <td class="dxgv" width="3%">
                  <a href="nota.php?action=R&id=' . $data["id_nota"] . '" class="" title="Editar">'
            . '<img title="Editar" border=0 src="../img/edit.png"></a>' . $borrarUceti;
            $result .= '<a href="javascript:exportar(' . $data["id_nota"] . ')" class="" title="Descarga Formulario">'
