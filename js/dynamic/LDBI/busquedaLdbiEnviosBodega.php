...
require_once("libs/helper/helperLdbiEnviosBodega.php");
...
$data = helperLdbiEnviosBodega::buscar($config);
$config["total"] = helperLdbiEnviosBodega::buscarCantidad($config);
...
$result.= '
            <tr>
                ...
                <th class="dxgvHeader_PlasticBlue" scope="">Fecha requesicion</th>
                <th class="dxgvHeader_PlasticBlue" scope="">Requesicion</th>
                <th class="dxgvHeader_PlasticBlue" scope="">Fecha envio</th>
                <th class="dxgvHeader_PlasticBlue" scope="">Envio</th>
...
if (is_array($data)) {
    foreach ($data as $data) {
        ...
        if ($permisoBorrar) {
            $borrarUceti = '<a href="javascript:borrar(' . $data["id_envio"] . ',' . $data["id_requesicion"] . ',\'' . $data["no_requesicion"] . '\')" class="" title="Borrar">'
        ...
        $result.='
                                ...
                                <td class="dxgv" width="3%">
                                    ' . ($data["fh_envio"] == "" ? "No disponible" : helperString::toDateView($data["fh_envio"])) . '
                                </td>
                                <td class="dxgv" width="5%">
                                ...
                                <td style="text-align: center" class="dxgv" width="3%">
                                    ' . ($data["fh_envio"] == "" ? "No disponible" : substr($data["fh_envio"],0,4)) . '
                                </td>
                                <td class="dxgv" width="8%">
                                    ' . $statusTexto . '
                                </td>
                                <td class="dxgv" width="3%">
                  <a href="envio.php?action=R&id=' . $data["id_envio"] . '" class="" title="Editar">'
            . '<img title="Editar" border=0 src="../img/edit.png"></a>' . $borrarUceti;
            $result .= '<a href="javascript:exportar(' . $data["id_envio"] . ')" class="" title="Descarga Formulario">'
