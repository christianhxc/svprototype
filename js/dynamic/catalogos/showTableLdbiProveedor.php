...
if ($mode == "delete") {
    $var_id = $_GET["id"];
    $error = dalCatalogos::borrarProveedorLdbi($var_id);
...
if ($mode == "new") {
    echo"<tr class=\"dxgvDataRow_PlasticBlue\">";
    echo"<td class=\"dxgv\" width=\"80%\"> <input  type=\"text\" maxlength=\"45\" nombre='nombre' size=\"55%\" id='n' /> </td>";
...
if ($mode == "save_new") {
    ...
    $s = $_REQUEST["s"];
//    $flu = $_REQUEST["f"];
    $id = dalCatalogos::agregarProveedorLdbi($n, $s);

    if ($id == "-1") {
        //$tabla.='</td></tr></table>';
        echo '<script type="text/javascript">';
        echo 'alert("Imposible agregar el proveedor, por favor intente nuevamente.");';
        echo '</script>';
    }
}
...
if ($mode == "update_data") {
    ...
    $error = dalCatalogos::editarProveedorLdbi($id, $n, $s);
...
$lista = dalCatalogos::obtenerLdbiProveedores($config);
...
if (isset($lista)) {
    foreach ($lista as $lista) {
        ...
        if ($id_ == $id) {
            ...
            echo "<td class=\"dxgv\" width= \"5%\"><a href=\"javascript:requestInfo('js/dynamic/catalogos/showTableLdbiProveedor.php?mode=delete&id=$id','showTable','')\" onclick='return confirmLink(this);'><img border=0 src=\"" . Configuration::getUrlprefix() . "img/iconos/delete.png\" title=\"Eliminar\"/></a></td>";
        }
        echo "</tr>";
    }
}
...
$total = dalCatalogos::conteoLdbiProveedores();
