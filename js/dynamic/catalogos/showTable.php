...
if ($mode == "delete") {
    $var_id = $_GET["id"];
    $error = dalCatalogos::borrarEnfermedadCronica($var_id);
...
if ($mode == "save_new") {
    ...
    $id = dalCatalogos::agregarEnfermedadCronica($n, $s, $flu);

    if ($id == "-1") {
        //$tabla.='</td></tr></table>';
        echo '<script type="text/javascript">';
        echo 'alert("Imposible agregar enfermedad cronica, por favor intente nuevamente.");';
        echo '</script>';
    }
}
...
if ($mode == "update_data") {
    ...
    $error = dalCatalogos::editarEnfermedadCronica($id, $n, $s, $flu);
...
$lista = dalCatalogos::obtenerEnfermedadesCronicas($config);
...
if (isset($lista)) {
    foreach ($lista as $lista) {
        ...
        if ($id_ == $id) {
            ...
            echo "<td class=\"dxgv\" width= \"5%\"><a href=\"javascript:requestInfo('js/dynamic/catalogos/showTable.php?mode=delete&id=$id','showTable','')\" onclick='return confirmLink(this);'><img border=0 src=\"" . Configuration::getUrlprefix() . "img/iconos/delete.png\" title=\"Eliminar\"/></a></td>";
        }
        echo "</tr>";
    }
}
...
$total = dalCatalogos::conteoEnfermedadesCronicas();
