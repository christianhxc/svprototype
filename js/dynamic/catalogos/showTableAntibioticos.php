...
if ($mode == "delete") {
    $var_id = $_GET["id"];
    $error = dalCatalogos::borrarAntibiotico($var_id);
...
if ($mode == "save_new") {
    ...
    $s = $_REQUEST["s"];
//    $flu = $_REQUEST["f"];
    $id = dalCatalogos::agregarAntibiotico($n, $s);

    if ($id == "-1") {
        //$tabla.='</td></tr></table>';
        echo '<script type="text/javascript">';
        echo 'alert("Imposible agregar el antibiotico, por favor intente nuevamente.");';
        echo '</script>';
    }
}
...
//    $flu = $_REQUEST["flu"];
    ...
    $error = dalCatalogos::editarAntibiotico($id, $n, $s);
...
$lista = dalCatalogos::obtenerAntibioticos($config);
...
//            echo "<td class=\"dxgv\" width= \"10%\">" . $influenza . "</td>";
            ...
            echo "<td class=\"dxgv\" width= \"5%\"><a href=\"javascript:requestInfo('js/dynamic/catalogos/showTableAntibioticos.php?mode=delete&id=$id','showTable','')\" onclick='return confirmLink(this);'><img border=0 src=\"" . Configuration::getUrlprefix() . "img/iconos/delete.png\" title=\"Eliminar\"/></a></td>";
        }
        echo "</tr>";
    }
}
...
$total = dalCatalogos::conteoAntibioticos();
