...
if ($mode == "delete") {
    $var_id = $_GET["id"];
    $error = dalCatalogos::borrarBodegaLdbi($var_id);
...
if ($mode == "save_new") {
    ...
    $c = $_REQUEST["c"];
//    $flu = $_REQUEST["f"];
    $id = dalCatalogos::agregarBodegaLdbi($n, $c, $s);

    if ($id == "-1") {
        //$tabla.='</td></tr></table>';
        echo '<script type="text/javascript">';
        echo 'alert("Imposible agregar la bodega, por favor intente nuevamente.");';
        echo '</script>';
    }
}
...
if ($mode == "update_data") {
    ...
    $error = dalCatalogos::editarBodegaLdbi($id, $n, $c, $s);
...
$lista = dalCatalogos::obtenerLdbiBodegas($config);
...
if (isset($lista)) {
    foreach ($lista as $lista) {
        ...
        if ($id_ == $id) {
            ...
            echo "<td class=\"dxgv\" width= \"5%\"><a href=\"javascript:requestInfo('js/dynamic/catalogos/showTableLdbiBodega.php?mode=delete&id=$id','showTable','')\" onclick='return confirmLink(this);'><img border=0 src=\"" . Configuration::getUrlprefix() . "img/iconos/delete.png\" title=\"Eliminar\"/></a></td>";
        }
        echo "</tr>";
    }
}
...
$total = dalCatalogos::conteoLdbiBodegas();
