...
if ($mode == "delete") {
    $var_id = $_GET["id"];
    $error = dalCatalogos::borrarVacuna($var_id);
...
if ($mode == "save_new") {
    ...
    $s = $_REQUEST["s"];
//    $flu = $_REQUEST["f"];
    $id = dalCatalogos::agregarVacuna($n, $c, $s);

    if ($id == "-1") {
        //$tabla.='</td></tr></table>';
        echo '<script type="text/javascript">';
        echo 'alert("Imposible agregar la vacuna, por favor intente nuevamente.");';
        echo '</script>';
    }
}
...
if ($mode == "update_data") {
    ...
    $error = dalCatalogos::editarVacuna($id, $n, $c, $s);
...
$lista = dalCatalogos::obtenerVacunas($config);
...
//        $influenza = ($influenza == '1' ? 'S&iacute;' : 'No');
        ...
        if ($id_ == $id) {
            ...
            echo "<td class=\"dxgv\" width= \"5%\"><a href=\"javascript:requestInfo('js/dynamic/catalogos/showTableVacunas.php?mode=delete&id=$id','showTable','')\" onclick='return confirmLink(this);'><img border=0 src=\"" . Configuration::getUrlprefix() . "img/iconos/delete.png\" title=\"Eliminar\"/></a></td>";
        }
        echo "</tr>";
    }
}
...
$total = dalCatalogos::conteoVacunas();
