...
if ($sistema == ConfigurationHospitalInfluenza::SISVIG) {
    $data = helperCatalogos::buscarPersonas($config);
    $config["total"] = helperCatalogos::buscarPersonasCantidad($config);
} else if ($sistema == ConfigurationHospitalInfluenza::SILABLOCAL) {
    $data = helperSilab::buscarPersonas($config);
    $config["total"] = helperSilab::buscarPersonasCantidad($config);
} else if ($sistema == ConfigurationHospitalInfluenza::SILABREMOTO) {
    $data = helperSilabRemoto::buscarPersonas($config);
    $config["total"] = helperSilabRemoto::buscarPersonasCantidad($config);
}
...
$result.= '
            <tr>
                ...
                <th class="dxgvHeader_PlasticBlue" scope="">Apellido Casada</th>
...
if (is_array($data)) {
    foreach ($data as $data) {
        ...
        if ($sistema == ConfigurationHospitalInfluenza::SISVIG) {
            $metodo = '"javascript:individuo(' . $data["tipo_identificacion"] . ',\'' . $data["numero_identificacion"] . '\')"';
        } else if ($sistema == ConfigurationHospitalInfluenza::SILABLOCAL) {
            $metodo = '"javascript:individuoSilab(' . $data["tipo_identificacion"] . ',\'' . $data["numero_identificacion"] . '\','.ConfigurationHospitalInfluenza::SILABLOCAL.')"';
        } else if ($sistema == ConfigurationHospitalInfluenza::SILABREMOTO) {
            $metodo = '"javascript:individuoSilab(' . $data["tipo_identificacion"] . ',\'' . $data["numero_identificacion"] . '\','.ConfigurationHospitalInfluenza::SILABREMOTO.')"';
        }
        ...
        $result.='<td class="dxgv" width="5%">
                                <a title="Seleccionar" class="ui-state-default ui-corner-all ui-link-button" onmouseover="RollOver(this)" onmouseout="RollOut(this)"
                                    href='.$metodo.'><span class="ui-icon ui-icon-check"></span></a>
                                </a>
                                </td>
                                ...
                                <td class="dxgv" width="10%">
                                    ' . ($data["segundo_apellido"] == "" ? "Sin dato" : htmlentities($data["casada_apellido"])) . '
                                </td>
