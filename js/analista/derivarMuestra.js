...
{
    if($("#evento_alicuota").val()!=0 && $("#area_alicuota").val()!=0)
    {
       $.ajax({
       type: "POST",
       url: urlprefix + 'js/dynamic/agregarDerivacion.php',
       data: "id="+ $("#muestra").val()+"&ev="+$("#evento_alicuota").val()+"&tip="+$("#idEventoTipoMuestra").val(),
       success: function(data)
       {
           ...
           else if(data=='2')
           {
               $("#mensajes").hide();
               $("#errores").show();
               $("#agregarError").html("La derivaci&oacute;n que seleccion&oacute; ya existe.");
           }
           else
           {
               $("#mensajes").hide();    
    ...
    else
   {
        $("#errores").show();
        $("#mensajes").hide();
        $("#agregarError").html("Por favor ingrese el &aacute;rea y el evento de la derivaci&oacute;n.");
   }
}
...
{
    if(confirm("\xbfEst\xe1 seguro(a) que desea anular esta derivaci\xf3n?"))
    {
       $.ajax({
       type: "POST",
