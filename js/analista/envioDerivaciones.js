...
$(document).ready(function()
{
    ...
    $("#area").change(function()
    {
        if(conteo!=0)
        {
            var retVal = confirm("Debe enviar derivaciones de un \xe1rea \xfanicamente, si cambia de \xe1rea se perder\xe1n las muestras de la lista de env\xedo. \xbfContinuar?");
...
function obtenerMuestras()
{
    $.ajax({
       type: "POST",
       url: urlprefix + 'js/dynamic/envioDerivaciones.php',
...
function seleccionarMuestra(numM)
{
    ...
    // si la derivación no está agregarla
    if(jQuery.inArray(numM, muestras)==-1)
    {
        muestras.push(numM);
        c1 = $("#g"+numM).text();
        c2 = $("#c"+numM).text();
        c3 = '<a title="Quitar derivaci&oacute;n del listado" class="ui-state-default ui-corner-all ui-link-button" onmouseover="RollOver(this)" onmouseout="RollOut(this)"'
        ...
        alert("La derivaci\xf3n que seleccion\xf3 ya se encuentra en la lista.");
...
function quitarMuestra(idMuestra)
{
    if(conteo>0)
    {
        var retVal = confirm("\xbfEliminar la derivaci\xf3n del listado de env\xedo?");
        if( retVal == true )
        {
            conteo--;
            $("#eR"+idMuestra).remove();
...
function Enviar()
{
    if(confirm("\xbfEnviar las al\xedcuotas seleccionadas?"))
    {
        var param = '';
        ...
        window.location = urlprefix+'analista/enviarDerivaciones.php?m='+param;
    }
}
