...
function buscar()
{
    ...
    $.ajax({
        type: "POST",
        url: urlprefix + 'js/dynamic/LDBI/busquedaLdbiEnviosBodega.php',
...
function borrar(id, req, texto){
    var mensaje = 'A continuaci\xf3n se borrara los datos del Envio N. '+texto+', \xbfdesea continuar?';
    if(confirm(mensaje)){
        window.location.replace('envio.php?action=D&id=' + id + '&req=' + req);
    }
}
...
function exportar(id){
    window.open('pdf/envio.php?id='+id,'_blank');
}