...
{
    var header = 'Imposible agregar al&iacute;cuota: <br/>'
    var mensaje = '';
    if($("#area_alicuota").val()==0)
        mensaje+='- Por favor seleccione el &aacute;rea de an&aacute;lisis<br/>';

    if($("#evento_alicuota").val()==0)
        mensaje+='- Por favor seleccione el evento de la al&iacute;cuota<br/>';

    if ($("#si").is(":checked"))
    {
        if($("#drprazonrechazo").val()==0)
            mensaje+='- Por favor especifique la raz&oacute;n de rechazo de la al&iacute;cuota<br/>';
    }

    if(mensaje=='')
    {
       var rechazada = (($("#si").is(":checked"))? 1 : 0);
       var razon = (rechazada == 0 ? 1 : $("#drprazonrechazo").val());

       $.ajax({
       type: "POST",
       url: urlprefix + 'js/dynamic/agregarAlicuota.php',
       data: "id="+ $("#muestra").val()+"&sec="+$("#area_alicuota").val()+"&ev="+$("#evento_alicuota").val()+"&tip="+$("#idEventoTipoMuestra").val()
       +"&r="+ rechazada + "&rr="+razon,
       success: function(data)
       {
           if(data=='1')
           {
               ...
               $("#mensajes").show();
               cargarDerivaciones();
               $("#exitoGuardar").html("Al&iacute;cuota agregada correctamente.");
           }
           else if(data=='2')
           {
               $("#mensajes").hide();
               $("#errores").show();
               $("#agregarError").html("La al&iacute;cuota que seleccion&oacute; ya existe.");
           }
           else
           {
               $("#mensajes").hide();    
...
function cargarDerivaciones()
{
       $.ajax({
       type: "POST",
       url: urlprefix + 'js/dynamic/obtenerAlicuotas.php',
       data: "id="+ $("#muestra").val(),
       success: function(data){
...
function eliminar(der)
{
    if(confirm("\xbfEst\xe1 seguro(a) que desea anular esta al\xedcuota?"))
    {
       $.ajax({
       type: "POST",
       url: urlprefix + 'js/dynamic/borrarAlicuota.php',
       data: "id="+ der,
       success: function(data)
       {
           if(data=='1')
           {
               $("#errores").hide();
               $("#mensajes").show();
               $("#exitoGuardar").html("&#161;Al&iacute;cuota anulada correctamente&#33;");
               cargarDerivaciones();
           }
           else if(data=='2')
           {
               $("#mensajes").hide();
               $("#errores").show();
               $("#agregarError").html("Imposible anular la al&iacute;cuota que seleccion&oacute; porque ya fue enviada.");
           }
           else if(data=='3')
           {
               $("#mensajes").hide();
               $("#errores").show();
               $("#agregarError").html("Imposible borrar al&iacute;cuota ingresada en ventanilla porque no ha sido confirmada recibida.");
           }
           else
           {
               $("#mensajes").hide();
