...
class ingresoVentanillaPage extends page
{
	public $config;
	...
	public function parseContent()
	{
		...
		$this->tpl->addBlockFile('CONTENT','contentBlock',Configuration::templatesPath.'ventanilla/ingresoVentanilla.tpl.html');

                // Para edición: deshabilita área de análisis, evento y tipo de muestra dependiendo
                // si tiene alícuotas ingresadas, o si ya fue enviada o si es alícuota la que se desea editar
                // ------------------------------------

                if(isset($this->config["muestra"]) && $this->config["muestra"]!='-1')
                {
                    $tipoMuestraEditable = helperMuestra::tipoMuestra($this->config["muestra"]);
                    
                    if($tipoMuestraEditable == Configuration::tipoAlicuotaIngresada)
                    {
                        $this->tpl->setVariable("disableEditing",'disabled');

                        $ubicacion = helperMuestra::situacionMuestra($this->config["muestra"]);
                        $ubicacion = $ubicacion[0];

                         // Está en ventanilla la alícuota?
                         if($ubicacion == Configuration::ventanilla)
                            $this->tpl->setVariable("disableEditingRazon",'');
                         else
                            $this->tpl->setVariable("disableEditingRazon",'disabled');
                    }
                    else if($tipoMuestraEditable == Configuration::tipoMuestra)
                    {
                        $cantidadHijas = count(helperMuestra::getHijas($this->config["muestra"]));
                        $ubicacion = helperMuestra::situacionMuestra($this->config["muestra"]);
                        $ubicacion = $ubicacion[0];
                        ...
                        if($cantidadHijas > 0){
                            ...
                            // si está en ventanilla puede editar la razón de rechazo
                             if($ubicacion == Configuration::ventanilla){
                                $this->tpl->setVariable("disableEditingRazon",'');
                             }
                             else {
                        ...
                        // No tiene hijas
                        {
                             // Está en ventanilla?
                             if($ubicacion == Configuration::ventanilla){
                                $this->tpl->setVariable("disableEditing",''); 
                                $this->tpl->setVariable("disableEditingRazon",'');
                             }
                             else {
                                $this->tpl->setVariable("disableEditing",'disabled');
                ...
                $areas = $this->config["catalogos"]["departamento"];
                if(is_array($areas))
                {
                        foreach($areas as $area)
                        {
                            $this->tpl->setCurrentBlock('blkAreaMuestra');
                            $this->tpl->setVariable("valAreaMuestra",$area["departamento"]);
                            $this->tpl->setVariable("opcAreaMuestra",htmlentities($area["descripciondepartamento"]));

                            if($this->config["preselect"])
                                $this->tpl->setVariable("selAreaMuestra",($area["departamento"]==$this->config["data"]["muestra"]["area_salud"]?'selected="selected"':''));
                ...
                if($nuevo && !$this->config["errorNuevo"])
                {
                    ...
                    $this->tpl->setVariable("chkNoLocalidadIndividuo",'');
...
//                    $this->tpl->setVariable('valEventoCorrelativo', '0');
                    ...
                    $this->tpl->setVariable("valMostrarFlu",'none');
                }
                else
                {
                    ...
                    if($this->config["data"]["individuo"]["confidencial"]=='1')
                    ...
                    // Nombres y apellidos
                    $this->tpl->setVariable('valPrimerNombre', htmlentities($this->config["data"]["individuo"]["primer_nombre"]));
                    $this->tpl->setVariable('valSegundoNombre', htmlentities($this->config["data"]["individuo"]["segundo_nombre"]));
                    
                    $this->tpl->setVariable('valPrimerApellido', htmlentities($this->config["data"]["individuo"]["primer_apellido"]));
                    $this->tpl->setVariable('valSegundoApellido', htmlentities($this->config["data"]["individuo"]["segundo_apellido"]));
                    $this->tpl->setVariable('valCasada', htmlentities($this->config["data"]["individuo"]["casada"]));
                    ...
                    $this->tpl->setVariable("valDireccionIndividuo",htmlentities($this->config["data"]["individuo"]["direccion"]));
                    ...
                    // NO localidad de individuo
                    if($this->config["data"]["individuo"]["no_localidad"]=='on')
                        $this->tpl->setVariable('chkNoLocalidadIndividuo', 'checked="checked"');
                    else
                        $this->tpl->setVariable('chkNoLocalidadIndividuo', '');
                    ...
                    if(is_array($departamentos))
                    {
                            ...
                            {
                                $this->tpl->setCurrentBlock('blkAreaMuestra');
                                $this->tpl->setVariable("valAreaMuestra",$departamento["departamento"]);
                                $this->tpl->setVariable("opcAreaMuestra",htmlentities($departamento["descripciondepartamento"]));

                                if($this->config["preselect"])
                                    $this->tpl->setVariable("selAreaMuestra",($departamento["departamento"]==$this->config["data"]["muestra"]["area_salud"]?'selected="selected"':''));
                                $this->tpl->parse('blkAreaMuestra');
                            }
                    }
                    
                    $distritos = helperLugar::getMunicipios($this->config["data"]["muestra"]["area_salud"]);
                    if(is_array($distritos))
                    {
                            foreach($distritos as $distrito)
                            {
                                $this->tpl->setCurrentBlock('blkDistritoMuestra');
                                $this->tpl->setVariable("valDistritoMuestra",$distrito["municipio"]);
                                $this->tpl->setVariable("opcDistritoMuestra",htmlentities($distrito["descripcionmunicipio"]));

                                if($this->config["preselect"])
                                    $this->tpl->setVariable("selDistritoMuestra",($distrito["municipio"]==$this->config["data"]["muestra"]["distrito"]?'selected="selected"':''));
                    ...
                    $this->tpl->setVariable('valServicioMuestra', $this->config["data"]["muestra"]["establecimiento"]);
                    ...
                    $param["idds"]=$this->config["data"]["muestra"]["distrito"];
...
//                    }
                    ...
                    $this->tpl->setVariable('valOtroEstablecimiento',htmlentities($this->config["data"]["muestra"]["otro_establecimiento"]));
...
//                    $this->tpl->setVariable('valServicioIntra',$this->config["data"]["muestra"]["servicio"]);
                    ...
                    $this->tpl->setVariable('valIniciosintomas',$this->config["data"]["muestra"]["evento"]["inicio_sintomas"]);
                    ...
                    // Si es flu mostrar ETI o IRAG
                    if($this->config["data"]["muestra"]["evento"]["id_evento"] == Configuration::FLU)
                    {
                        $this->tpl->setVariable("valMostrarFlu",'');
                        // Selecciona la opción
                        if($this->config["data"]["muestra"]["flu"]==1)
                        {
                            $this->tpl->setVariable("flu1",'selected');
                            $this->tpl->setVariable("flu2",'');
                        }
                        else
                        {
                            $this->tpl->setVariable("flu1",'');
                            $this->tpl->setVariable("flu2",'selected');
                        }
                    }
                ...
                if(clsCaus::validarSeccion(ConfigurationCAUS::v1, ConfigurationCAUS::Agregar))
                    $this->tpl->setVariable("botonGuardar",'<a href="javascript:validarIngreso();" class="ui-state-default ui-corner-all ui-button" onmouseover="RollOver(this)" onmouseout="RollOut(this)"><span class="ui-icon ui-icon-disk"></span> Guardar</a>&nbsp;');
                if(clsCaus::validarSeccion(ConfigurationCAUS::v1, ConfigurationCAUS::Consultar))
                    $this->tpl->setVariable("botonCancelar",'<a href="../ventanilla/index.php" class="ui-state-default ui-corner-all ui-button" onmouseover="RollOver(this)" onmouseout="RollOut(this)">                                <span class="ui-icon ui-icon-cancel"></span> Cancelar </a>');
