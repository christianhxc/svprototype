...
class vicItsPage extends page {
    ...
    public function parseContent() {
        $this->tpl->addBlockFile('CONTENT', 'contentBlock', Configuration::templatesPath . 'vicits/buscarVicIts.tpl.html');
...
//        $this->tpl = TemplateHelp::getPaginator($this->tpl, $this->config);
        ...
        if (clsCaus::validarSeccion(ConfigurationCAUS::vicItsFormulario, ConfigurationCAUS::Borrar))
            $this->tpl->setVariable("valPermisoBorrar", 'si');
        else 
            $this->tpl->setVariable("valPermisoBorrar", 'no');
        if (clsCaus::validarSeccion(ConfigurationCAUS::vicItsFormulario, ConfigurationCAUS::Reportes))
