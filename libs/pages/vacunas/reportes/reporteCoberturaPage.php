...
class reporteCoberturaPage extends page
{
    public $config;
    ...
    {
        $this->tpl->addBlockFile('CONTENT','contentBlock',Configuration::templatesPath.'vacunas/reportes/reporteCobertura.tpl.html');


        // carga configuraciones guardadas
        $configuraciones = dalReportesConfiguracion::Leer();
        if (is_array($configuraciones)) {
            foreach ($configuraciones as $conf) {
                $this->tpl->setCurrentBlock('blkConfiguraciones');
                $this->tpl->setVariable("valConfiguracion", $conf["id_configuracion"]);
                $this->tpl->setVariable("opcConfiguracion", htmlentities($conf["nombre"]));

                $this->tpl->parse('blkConfiguraciones');
            }
        }
...
//                if(clsCaus::validarSeccion(ConfigurationCAUS::r4, ConfigurationCAUS::Reportes))
        $this->tpl->setVariable("botonGenerar",'<input type="button" value="Generar reporte" id="generar-reporte" class="ui-state-default ui-corner-all ui-button" onmouseover="RollOver(this)" onmouseout="RollOut(this)" title="Por favor considere que el reporte puede tardarse" />');
