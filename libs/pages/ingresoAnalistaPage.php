...
class ingresoAnalistaPage extends page
{
	public $config;
	...
	public function parseContent()
	{
		...
		$this->tpl->addBlockFile('CONTENT','contentBlock',Configuration::templatesPath.'analista/ingresoAnalista.tpl.html');
...
//                    $this->tpl->setVariable('valEventoCorrelativo', '0');
                ...
                else
                {
                    ...
                    // Despliega como texto el área de análisis que no puede cambiar
                    if(is_array($areas))
                    {
                        for($i=0; $i<count($areas); $i++)
                        {
                            if($areas[$i]["ARE_ANA_ID"]==$this->config["data"]["muestra"]["area_analisis"])
                            {
                                $this->tpl->setVariable('opcArea',$areas[$i]["ARE_ANA_NOMBRE"]);
                                break;
                            }
                        }
                    }
                    ...
                    if(is_array($eventos))
                    {
                        ...
                        {
                            if($eventos[$i]["eve_id"]==$this->config["data"]["muestra"]["evento"]["id_evento"])
                            {
                                $this->tpl->setVariable('opcEvento',$eventos[$i]["eve_nombre"]);
                                break;
                            }
                        }
                    }
                    ...
                    $this->tpl->setVariable("valTipoMuestraChosen",$this->config["data"]["muestra"]["tipo"]);
