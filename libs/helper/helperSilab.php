...
class helperSilab {
    ...
    public static function getDatosBasicosDerivacion($id) {
        ...
        $sql = "SELECT
            ...
            FROM
              derivacion
              LEFT JOIN muestra ON (derivacion.MUE_ID = muestra.MUE_ID)
              LEFT JOIN tipo_muestra_evento ON (derivacion.TIP_MUE_EVE_ID = tipo_muestra_evento.TIP_MUE_EVE_ID)
              LEFT JOIN evento ON (tipo_muestra_evento.EVE_ID = evento.EVE_ID)
              LEFT JOIN area_analisis ON (evento.ARE_ANA_ID = area_analisis.ARE_ANA_ID)
              LEFT JOIN tipo_muestra ON (tipo_muestra_evento.TIPO_MUE_ID = tipo_muestra.TIP_MUE_ID)
              LEFT JOIN estado_motivo ON (derivacion.EST_MOT_ID = estado_motivo.EST_MOT_ID)
    ...
    public static function getPruebasRealizadas($id) {
        ...
        $sql = "SELECT
            ...
            FROM
              analisis_muestra
              LEFT JOIN prueba_resultado ON (analisis_muestra.PRU_RES_ID = prueba_resultado.PRU_RES_ID)
              LEFT JOIN resultado ON (prueba_resultado.RES_ID = resultado.RES_ID)
              LEFT JOIN prueba_evento ON (prueba_resultado.PRU_EVE_ID = prueba_evento.PRU_EVE_ID)
              LEFT JOIN prueba ON (prueba_evento.PRU_ID = prueba.PRU_ID)
              LEFT JOIN tipo_muestra_evento ON (prueba_evento.TIP_MUE_EVE_ID = tipo_muestra_evento.TIP_MUE_EVE_ID)
              LEFT JOIN tipo_muestra ON (tipo_muestra_evento.TIPO_MUE_ID = tipo_muestra.TIP_MUE_ID)
...
//        echo '<pre>'; print_r($data); echo'</pre>'; exit;
    ...
    public static function getConclusionMuestra($idMuestra) {
        ...
        $sql = "SELECT
                ...
                FROM
                  conclusion_muestra
                  LEFT JOIN muestra ON (conclusion_muestra.MUE_ID = muestra.MUE_ID)
                  LEFT JOIN conclusion ON (conclusion_muestra.TIP_SUB_ID1 = conclusion.TIP_SUB_ID)
                  LEFT JOIN conclusion conclusion1 ON (conclusion_muestra.TIP_SUB_ID2 = conclusion1.TIP_SUB_ID)
    ...
    public static function getPruebasRealizadasDerivacion($id) {
        ...
        $sql = "SELECT
            ...
            FROM
              analisis_derivacion
              LEFT JOIN prueba_resultado ON (analisis_derivacion.PRU_RES_ID = prueba_resultado.PRU_RES_ID)
              LEFT JOIN resultado ON (prueba_resultado.RES_ID = resultado.RES_ID)
              LEFT JOIN prueba_evento ON (prueba_resultado.PRU_EVE_ID = prueba_evento.PRU_EVE_ID)
              LEFT JOIN prueba ON (prueba_evento.PRU_ID = prueba.PRU_ID)
              LEFT JOIN tipo_muestra_evento ON (prueba_evento.TIP_MUE_EVE_ID = tipo_muestra_evento.TIP_MUE_EVE_ID)
    ...
    public static function getMetodologias($prueba) {
        ...
        $sql = " SELECT
                 ...
                 FROM
                    ...
                    LEFT join metodologia m on m.idmetodologia = pm.idmetodologia
                WHERE PRU_ID=" . $prueba;
    ...
    public static function getMuestras($config) {
        ...
        $sql = 'select mue.MUE_ID as "id_muestra", 
                ...
                from muestra mue
                LEFT join individuo ind ON mue.IND_ID = ind.IND_ID
                LEFT join tipo_muestra_evento tme ON mue.TIP_MUE_EVE_ID = tme.TIP_MUE_EVE_ID
    ...
    public static function traerAllDatosSilab() {
        $conn = new ConnectionSilab();
        $conn->initConn();
        $sql = "select mue.MUE_ID, mue.MUE_CODIGO_GLOBAL_ANIO, mue.MUE_CODIGO_GLOBAL_NUMERO,
                per.IND_PRIMER_NOMBRE, per.IND_SEGUNDO_NOMBRE, per.IND_PRIMER_APELLIDO, per.IND_SEGUNDO_APELLIDO, 
                per.IND_IDENTIFICADOR, 
                (case when (per.IND_IDENTIFICADOR_TIPO = 1) then 1 else 2 end) AS IND_IDENTIFICADOR_TIPO,
                tip.nombre as nombre_tipo,
                per.IND_PROC_PROVINCIA, per.IND_PROC_REGION, per.IND_PROC_DISTRITO, 
                per.IND_PROC_CORREGIMIENTO, per.IND_DIRECCION,
                per.IND_TELEFONO,per.IND_FECHA_NACIMIENTO, per.IND_EDAD, per.IND_TIPO_EDAD, per.IND_SEXO,
                mue.MUE_FECHA_INICIO, 
                (case when (mue.MUE_SITUACION_HOSPITALAR = 2) then 2 else 1 end) AS condicion,
                mue.MUE_PROC_INST_SALUD, un.cod_ref_minsa, mue.MUE_OTRO_ESTABLECIMIENTO_NOMBRE,
                mue.MUE_REFERIDA_POR, mue.MUE_FECHA_INGRESO_SISTEMA, mue.MUE_SEMANA_EPI
                from muestra mue
                LEFT join individuo per on per.IND_ID = mue.IND_ID
                LEFT join tipo_identificacion tip on mue.IND_IDENTIFICADOR_TIPO = tip.idtipoidentificacion
                LEFT join tipo_muestra_evento teve on teve.TIP_MUE_EVE_ID = mue.TIP_MUE_EVE_ID
                LEFT join conclusion_muestra con on con.MUE_ID = mue.MUE_ID
                LEFT join tipo_subtipo sub on sub.TIP_SUB_ID = con.TIP_SUB_ID1
                LEFT join resultado_final_tipo tipo on tipo.RES_FIN_TIP_ID = sub.RES_FIN_TIP_ID
                LEFT join resultado_final_evento res_fin on res_fin.RES_FIN_EVE_ID = tipo.RES_FIN_EVE_ID
                LEFT join resultado_final final on final.RES_FIN_ID = res_fin.RES_FIN_ID
                LEFT join cat_unidad_notificadora un on un.id_un = mue.MUE_PROC_INST_SALUD
                where teve.EVE_ID = 49 and res_fin.RES_FIN_ID = 1 and mue.IND_IDENTIFICADOR_DISPONIBLE = 1
                order by mue.MUE_CODIGO_GLOBAL_ANIO, mue.MUE_ID";
    ...
    public static function traerAllDatosSilabFactores() {
        $conn = new ConnectionSilab();
        $conn->initConn();
        $sql = "select 
                MUE_ID,
                ...
                sum(UDI) AS UDI,
                sum(UDO) AS UDO,
                sum(no_preservativo) AS no_preservativo,
                sum(exp_perinatal) AS exp_perinatal,
                sum(desconocido) AS desconocido,
                sum(embarazo) AS embarazo,
                sum(donante) AS donante,
                sum(ITS) AS ITS,
                sum(transfusion) AS transfusion,
                sum(cont_vih) AS cont_vih
                from view_factor_riesgo
                group by MUE_ID;";
