...
require_once ('libs/pages/LDBI/formLDBIEnvioPage.php');
...
if (!isset($_REQUEST["action"])) {
    ...
    if (count($_POST)) {
        ...
        $mensaje = dalLdbi::GuardarEnvioBodega($_POST["data"]);
        if ($mensaje == 1) {
            $config["info"] = Etiquetas::mensajeInfo;
            header("location: envios.php?info=" . $config["info"]);
        } else if ($mensaje == 2) {
    ...
    if (count($_POST)) {
        $config["data"] = $_POST["data"];
        $mensaje = dalLdbi::ActualizarEnvioBodega($_POST["data"]);
        if ($mensaje == 1) {
    ...
    if (isset($_REQUEST["id"])) { // D borrar
        ...
        $mensaje = dalLdbi::BorrarEnvioBodega($config);
        if ($mensaje == 1) {
            $config["info"] = Etiquetas::mensajeInfoBorrar;
            header("location: envios.php?info=" . $config["info"]);
        } else {
            $config["Merror"] = Etiquetas::mensajeErrorBorrar;
            header("location: envios.php?info=" . $config["Merror"]);
        }
    }
...
$page = new formLDBIEnvioPage($config);
$page->displayPage();
?>
